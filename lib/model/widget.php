<?php
load(array("model/board", "model/section", "model/favor", "inc/db"));
...
class Widget {
    ...
    public static function wInit($user){
        try{
            $uid = $user->userid;
            $db = DB::getInstance();
            $default = c("widget.default");
            foreach($default as $k=>$v){
                $w = self::getInstance($k);
                $color = isset($v["color"])?$v["color"]:0;
                $title = !empty($v["title"])?$v["title"]:$w->wGetTitle();
                $arr[] = array('uid' => $uid, 'wid' => $w->wGetName(), 'title' => $title['text'], 'color' => $color, 'col' => $v["col"], 'row' => $v["row"]);
                $ret[] = array(
                    "color"=> $color,
                    "title"=> nforum_html($title['text']),
                    "url"=> $title['url'],
                    "wid"=>$w->wGetName(),
                    "col" => $v['col'],
                    "row" => $v['row']
    ...
    public static function wGet($user){
        $colors = c("widget.color");
        $uid = $user->userid;
        $ret = array();
        if($uid == 'guest'){
            $widgets = c("widget.default");
    ...
    public static function wAdd($user, $wid, $title, $color, $col, $row){
        ...
        try{
            ...
            $val = array('uid' => $uid, 'wid' => $wid, 'title' => $title, 'color' => $color, 'col' => $col, 'row' => $row);
