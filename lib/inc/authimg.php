...
class NF_AuthImg {
    ...
    public function getImage(){
        ...
        $this->_session['authNum'] = $res;
        $font =  LIB . DS . 'inc' . DS . 'font' . DS . mt_rand(0, 10) . ".ttf";
        $img = new AuthImg();
        $img->setFormat('jpeg');
        $img->setWH(120, 40);
        $img->setFont($font);
    ...
    public function check($num){
        if(isset($this->_session['authNum']) && intval($num) == $this->_session['authNum'])
            return true;
        return false;
    }

    public function destory(){
        unset($this->_session['authNum']);
    }
}
