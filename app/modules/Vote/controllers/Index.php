...
class IndexController extends NF_Controller {
    ...
    public function init(){
        ...
        try{
            load('model/board');
            Board::getInstance($this->_board);
        }catch(BoardNullException $e){
            $this->error('请先建立版面: ' . $this->_board);
        }
    ...
    public function indexAction(){
        ...
        if(isset($this->params['url']['s'])){
            ...
            if('' != $search){
                $this->set('search', nforum_html($search));
        ...
        switch($category){
            ...
            case 'list':
                ...
                $title = ($user === $u->userid)?"我的投票":nforum_html("{$user}的投票");
                $this->set("vote_user", nforum_html($user));
    ...
    public function ajax_addAction(){
        ...
        $site = c("site");
        $a_title = $subject;
        $a_content = "主题:$subject\n描述:$desc\n发起人:{$u->userid}\n类型:".(($type==0)?'单选':'多选')."\n截止日期:$end\n链接:[url={$site['domain']}{$site['base']}/vote/view/$vid]{$site['domain']}{$site['base']}/vote/view/{$vid}[/url]\n[vote=$vid][/vote]";
        load("model/article");
    ...
    public function viewAction(){
        ...
        $item = $vote->items;
        foreach($item as $k=>$v){
            $item[$k]["label"] = nforum_html($v["label"]);
        ...
        try{
            ...
            foreach($articles as $v){
                ...
                try{
                    ...
                    $tmp['uid'] = $own->userid;
                    $tmp['furl'] = nforum_html($own->getFace());
                }catch(UserNullException $e){
                    $tmp['uid'] = $v->OWNER;
                    $tmp['furl'] = false;
                }
