...
class Vote{
    ...
    public static function add($uid, $subject, $desc, $end, $type, $limit,  $items, $result_voted = 0){
        $db = DB::getInstance();
        $val = array('uid' => $uid, 'subject' => $subject, 'desc' => $desc, 'start' => time(), 'end' => $end
            , 'type' => $type, 'limit' => $limit, 'status' => 1, 'result_voted' => $result_voted);
        $db->insert('pl_vote', $val);
        $vid = $db->lastInsertId();
        $val = array();
        foreach($items as $v){
            $val[] = array('vid' => $vid, 'label' => $v);
        }
    ...
    public function vote($uid, $items){
        ...
        if($this->type == 0){
            ...
            $val = array('vid' => $this->vid, 'uid' => $uid, 'result' => $items, 'time' => time());
            ...
            $val = array('vid' => $this->vid, 'uid' => $uid, 'result' => join("|", $items), 'time' => time());
            $db->insert("pl_vote_result", $val);
        }
    }
