<?php
class WeatherShell extends NF_Shell {
        ...
        for($j = 0; $j <=2; $j++){
            ...
            for($i=0;$i<=count($vals)-1;$i++){
                if($vals[$i]['tag'] == "status1" && $vals[$i]['type'] == "complete"){
                    $curStatB = nforum_iconv("utf-8",'GBK',$vals[$i]['value']);
                }
                if($vals[$i]['tag'] == "status2" && $vals[$i]['type'] == "complete"){
                    $curStatE = nforum_iconv("utf-8",'GBK',$vals[$i]['value']);
                }
                ...
                if($vals[$i]['tag'] == "power1" && $vals[$i]['type'] == "complete"){
                    $curWind1 = nforum_iconv("utf-8",'GBK',$vals[$i]['value']);
                }
                if($vals[$i]['tag'] == "power2" && $vals[$i]['type'] == "complete"){
                    $curWind2 = nforum_iconv("utf-8",'GBK',$vals[$i]['value']);
                }
                if($vals[$i]['tag'] == "zwx_l" && $vals[$i]['type'] == "complete"){
                    $zwx = nforum_iconv("utf-8",'GBK',$vals[$i]['value']);
                }
