    ...
    this.each(function(){
        ...
        TREE.setAjaxNodes = function(obj)
        {
            ...
            if(url && url.indexOf('url:'))
            {
                ...
                $.getJSON(url,function(responce){
                    if(responce instanceof Array && responce.length > 0)
                    {
                        obj.removeClass('ajax');
                        responce = TREE.makeHtmlJson(responce); 
                        obj.html(responce);
        ...
        TREE.serialize = function(obj){
            if($(obj).parent().parent().get(0) == TREE){
                ...
                $.cookie(TREE.option.cookie, data.join(""),{path:'/', domain:config.domain,expires:30});
            }
        };
