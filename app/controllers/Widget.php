 ...
 */
load('model/widget');
class WidgetController extends NF_Controller {

    public function ajax_widgetAction(){
        if(!isset($this->params['name']))
            $this->error();
        $name = $this->params['name'];
        if(!NF_Session::getInstance()->isLogin && !in_array($name, array_keys(c("widget.default"))))
            $this->error();
        try{
...
<?php
     ...
     * co: color num
     */
    public function ajax_setAction(){
        ...
        switch($t){
            case Widget::$ADD:
                ...
                try{
                    ...
                    $color = ($color < count(c("widget.color")) && $color >= 0)?$color:0;
